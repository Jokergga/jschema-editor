...
export default class SchemaDescription {
  ...
  changeName({
    ...
    // const openKeys = [].concat(keys as any, value, 'properties').join(JSONPATH_JOIN_CHAR);
    const openKeys = [...keys, value, 'properties'].join(JSONPATH_JOIN_CHAR);
    const oldOpenKeys = [...keys, name, 'properties'].join(JSONPATH_JOIN_CHAR);
